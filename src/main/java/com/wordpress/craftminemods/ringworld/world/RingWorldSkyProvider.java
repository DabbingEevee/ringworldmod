...
public class RingWorldSkyProvider extends IRenderHandler{
    //private static final ResourceLocation ringTexture = new ResourceLocation(Info.MODID, "textures/other/ring.png");

	//private static ResourceLocation purgotTexture = new ResourceLocation("zollerngalaxy:textures/gui/purgot.png");
	private static final ResourceLocation corTexture = new ResourceLocation(Info.MODID, "textures/other/cor.png");
	private static final ResourceLocation ringTexture2 = new ResourceLocation(Info.MODID, "textures/other/ringsegment_2.png");
	...
	public void render(float partialTicks, WorldClient world, Minecraft mc) {
		...
		Vec3d vec3 = new Vec3d(1.0000000000f, 0.8f, 0.8f);
		...
		float r = 255;// afloat[0] * f18;
		float g = 58;// afloat[1] * f18;
		float b = 3;// afloat[2] * f18 / 20.0F;
		float a = afloat[3] * 2 / f18;
		worldRenderer1.pos(0.0D, 100.0D, 0.0D).color(r, g, b, a).endVertex();
		byte b0 = 16;
		r = 255;// afloat[0] * f18;
		g = 58;// afloat[1] * f18;
		b = 3;// afloat[2] * f18 / 20.0F;
		a = 0.0F;
		*/
		/*/ Render sun aura
		f10 = 20.0F;
		...
		*/
		...
		f10 = 55;
		mc.renderEngine.bindTexture(RingWorldSkyProvider.ringTexture2);
		...
		// GL11.glColor4f(0.0F, 0.0F, 0.0F, 1.0F);// TODO
		f10 = this.sunSize;
		mc.renderEngine.bindTexture(RingWorldSkyProvider.corTexture);
		worldRenderer1.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX);
		worldRenderer1.pos(-f10, 100.0D, -f10).tex(0.0D, 0.0D).endVertex();
		worldRenderer1.pos(f10, 100.0D, -f10).tex(1.0D, 0.0D).endVertex();
		worldRenderer1.pos(f10, 100.0D, f10).tex(1.0D, 1.0D).endVertex();
		...
		FMLClientHandler.instance().getClient().renderEngine.bindTexture(RingWorldSkyProvider.purgotTexture);
		worldRenderer1.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX);
		...
		*/
		GL11.glPushMatrix();
		GL11.glTranslatef(0.0F, -((float) (0 - 16.0D)), 0.0F);
		GL11.glCallList(this.glSkyList2);
		GL11.glPopMatrix();
		GL11.glEnable(GL11.GL_TEXTURE_2D);
	...
	private void renderStars() {
		...
		for (int starIndex = 0; starIndex < (true ? 35000 : 6000); ++starIndex) {
    ...
    public float calculateTimeCorrectly(long worldTime, float partialTicks)
    {
	...
	public float getSkyBrightness(float par1) {
		final float var2 = calculateTimeCorrectly(FMLClientHandler.instance().getClient().world.getWorldTime(), 0.0f);
		float var3 = 1.0F - (MathHelper.sin((float) (var2 * 6.28318531)) * 2.0F + 0.25F);
	...
	public void onClientTick(ClientTickEvent event) {
		...
		if (world.provider instanceof RingWorldWorldProvider)
		{
			if (world.provider.getSkyRenderer() == null)
			{
				world.provider.setSkyRenderer(new RingWorldSkyProvider());
			}
		}		
	}*/
}
